...
- 文档引用：在节点中输入 `[[` 可从文档库补全并插入 `[[文档名]]` 内部引用；引用保存稳定文档 ID，点击可直接切换到目标文档，目标文件移动后会尝试通过文档库重新定位。文档库会把这些引用同步为可重建的 SQLite 派生索引，并提供反向引用（Backlinks）以及 incoming / outgoing / both 一跳关系查询，为后续全局/局部知识图谱视图提供统一数据层。
